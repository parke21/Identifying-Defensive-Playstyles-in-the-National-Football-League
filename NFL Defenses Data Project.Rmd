...
# Step 1: Convert rownames of team_features to a column
team_features$defensiveTeam <- rownames(team_features)

# Step 2: Merge on defensiveTeam
combined_team_stats <- merge(team_features, team_stats, by = "defensiveTeam")

# Optional: reorder by a specific metric (e.g., pressure_rate)
...
set.seed(123)
wss <- sapply(1:10, function(k) {kmeans(scaled_features, centers = k, nstart = 25)$tot.withinss})
...
###
# Suppose 'team_features' is your numeric, team-level data frame
pca_result <- prcomp(combined_team_stats, scale. = TRUE)
# Scree plot
plot(pca_result)

# Compute variance explained from PCA result
pca_var <- pca_result$sdev^2
pca_var_pct <- 100 * pca_var / sum(pca_var)

# Plot % variance explained
plot(pca_var_pct, pch = 16,
